...
def get_access_code(doctype, docname):
	...
	# redirect_uri = 'http://0.0.0.0:8001/api/method/dsc_erpnext.dsc_apis.auth_login'
	redirect_uri = 'http://127.0.0.1:8001/api/method/dsc_erpnext.dsc_api.auth_login'
...
def auth_login():
	...
	if data:
		...
		# get_signing_url(data['doctype'],data['docname'],data['access_token'],data['code'])
...
def get_signing_url(doctype, docname, token, code):
    try:
        ...
        args = {
            "signer_email": "sonukumarcode@gmail.com",
            "signer_name": 'sonu kumar',
            "client_id": frappe.db.get_single_value('Docusign Settings', 'integration_key'),
        ...
        frappe.local.response['location'] = res["uri"]
        frappe.db.commit()
    except Exception as e:
        frappe.logger("docusign").exception(e)
        return f"Error: {e}"
        ...
        "documents": [
            {
                ...
                "documentId": 1,
                "fileExtension": "pdf",
                "name": "for sign document"
            }
        ],
        "emailSubject": "Please sign this document sent from the Python SDK",
        "recipients": {
            "signers": [
                {
...
def sign_document(**kwargs):
	data = kwargs
	res_data = data["data"]
	docusign_envelope_id = res_data["envelopeId"]
	...
	get_signed_document(doctype=childValue.parenttype, docname=childValue.parent)
	frappe.log_error(title="sign document", message="Docusign is working", reference_doctype=childValue.parenttype, reference_name=childValue.parent)
	frappe.db.commit()
	return
